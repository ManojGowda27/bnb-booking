...
func TestForm_Valid(t *testing.T) {
	...
	if !isValid {
		t.Error("Got Invlid when it should have been valid")
	}
}
...
func TestForm_MinLength(t *testing.T) {
	...
	postedValues = url.Values{}
	postedValues.Add("another_field", "Manoj")
	form = New(postedValues)

	form.MinLength("another_field", 1)
	if !form.Valid() {
		t.Error("shows minlength of 1 is not met when data is longer")
	}
...
func TestForm_IsEmail(t *testing.T) {
	...
	postedValues = url.Values{}
	postedValues.Add("email", "mg@vm.com")
	form = New(postedValues)
